...
def get_usage_client(
    host="mysite.hosted.panopto.com", username="admin", password="yoursecrethere"
):
...
def show_report_status(usage_client, report_type="FolderUsage"):
    report_status = usage_client.call_service("GetRecentReports", reportType=report_type)
...
def queue_report(usage_client, start, end, report_type="FolderUsage"):
...
def read_report(usage_client, report_id):
    raw_response = usage_client.call_service_raw("GetReport", reportId=report_id)
