...
auth = AuthenticatedClientFactory(
    host, cookie, oauth_token, username, password, verify_ssl=host != "localhost"
)
...
if not reports:
    ...
    report_id = usage.call_service(
        "QueueReport", reportType="SessionUsage", startTime=last_month, endTime=now
    )
    ...
    for report in reports:
        print(
            "\t{ReportId}: {StartTime} - {EndTime} ".format(**report)
...
if not report_id:
    ...
    report_rows = [
        row.decode("utf-8") for row in zip_archive.open(report_file).readlines()
    ]
    ...
    if len(report_rows) == 1:
        ...
        print(
            "most-viewed session: {} ({} views)".format(
                mvs[session_name_index], mvs[views_index]
            )
        )
        # highest-rated session
        hrs = max(rows, key=lambda r: r[average_rating_index])
        print(
            "highest-rated session: {} (rated {} out of 5)".format(
        ...
        presenter_stats = {
            p: {c: 0 for c in presenter_column_indices}
        ...
        for stat_name, stat_description, stat_formatter in [
            (
                "Views and Downloads",
                "most-viewed presenter by view count",
                lambda s: int(s),
            ),
            ("Minutes Delivered", "most-viewed presenter by minutes viewed", lambda s: s),
            (
                "Unique Viewers",
                "most-viewed presenter by unique viewers",
                lambda s: int(s),
            ),
            (
                "Session Length",
                "most prolific presenter by minutes presented",
                lambda s: s,
            ),
        ]:
            ...
            print(
                "{}: {} ({})".format(
                    stat_description, winner, stat_formatter(details[stat_name])
                )
            )
