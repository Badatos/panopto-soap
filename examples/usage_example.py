...
if gudu_response["TotalNumberResponses"] > 0:
    ...
    gudu_response = usage.call_service(
        ...
        pagination={
            ...
            "PageNumber": int(
                ceil(gudu_response["TotalNumberResponses"] / float(page_size)) - 1
            ),
        },
    )
    ...
    week_views = [
        v
        for v in gudu_response["PagedResponses"]["DetailedUsageResponseItem"]
        if v["Time"].replace(tzinfo=None) > beginRange
    ]
    if week_views:
        ...
        for day in sorted(view_days, key=lambda d: d["Time"]):
            day_offset = int(
                ceil(
                    (endRange - day["Time"].replace(tzinfo=None)).total_seconds()
                    / (3600 * 24)
                )
            )
            print(
                "{} day{} ago: {} unique users viewed {} minutes in {} distinct views".format(
                    day_offset,
